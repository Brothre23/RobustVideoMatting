...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        attn_windows = self.attn(x_windows,
                                 mask=attn_mask)  # B*nW, Wd*Wh*Ww, C
        # merge windows
...
class SwinTransformerEncoder(SwinTransformer3D):
    def __init__(self):
        super().__init__()
    ...
    def forward_time_series(self, x):
        ...
        x = self.patch_embed(x)
        f1 = rearrange(x, 'n c d h w -> n d c h w')
        x = self.layers[0](x.contiguous())
        f2 = rearrange(x, 'n c d h w -> n d c h w')
        x = self.layers[1](x.contiguous())
        f3 = rearrange(x, 'n c d h w -> n d c h w')
        x = self.layers[2](x.contiguous())
        x = self.layers[3](x.contiguous())
        f4 = rearrange(x, 'n c d h w -> n d c h w')
    ...
    def forward_single_frame(self, x):
        ...
        features = self.forward_time_series(x)
