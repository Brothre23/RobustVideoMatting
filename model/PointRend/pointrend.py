...
class PointRendRefiner(nn.Module):
    ...
    def forward_single_frame(self, src, hid, fgr, pha, last, T):
        """
        ...
        return fgr, pha, torch.unsqueeze(pha[-1, :, :, :], dim=0)

    def forward_time_series(self, src, hid, fgr, pha, last):
        B, T = src.shape[:2]
        fgr, pha, last = self.forward_single_frame(
            src.flatten(0, 1),
            hid.flatten(0, 1),
            fgr.flatten(0, 1),
            pha.flatten(0, 1),
    ...
    def forward(self, src, hid, fgr, pha, last):
        if src.ndim == 5:
            return self.forward_time_series(src, hid, fgr, pha, last)
        else:
            return self.forward_single_frame(src, hid, fgr, pha, last, -1)
